...
-- 2. Drop the existing overly permissive RLS policy on co_authors
DROP POLICY IF EXISTS "Users can view co-author names, admins see all data" ON public.co_authors;

-- 3. Create restrictive RLS policies on co_authors table
-- Only admins can view full co_authors table data
...
-- This requires joining with research_papers table to check status
...
-- Allow anonymous users to view published papers
